...
def measure_retained_footprint(obj, seen=None) -> int:
    """
    Recursively computes the deep retained memory footprint of an object in bytes,
    accounting for slotted instances, containers, nested dataclasses, and references.
    """
    if seen is None:
        seen = set()
    obj_id = id(obj)
    ...
    if isinstance(obj, dict):
        size += sum(measure_retained_footprint(k, seen) + measure_retained_footprint(v, seen) for k, v in obj.items())
    elif hasattr(obj, '__dict__'):
        size += measure_retained_footprint(obj.__dict__, seen)
    elif hasattr(obj, '__slots__'):
        for slot in obj.__slots__:
            if hasattr(obj, slot):
                val = getattr(obj, slot)
                if val is not None:
                    size += measure_retained_footprint(val, seen)
    elif isinstance(obj, (list, tuple, set, frozenset)):
        size += sum(measure_retained_footprint(item, seen) for item in obj)
...
def run_memory_benchmark():
    ...
    # Retain active populated structure in scope
    leaves = quadtree.build(pts)
    active_cell_count = len(leaves)

    # Explicitly capture retained in-memory footprint (including coarse pool, leaves, slots, and stats)
    retained_bytes = measure_retained_footprint(quadtree)
    quadtree_ram_mb = retained_bytes / (1024.0 * 1024.0)
    effective_bytes_per_cell = retained_bytes / max(active_cell_count, 1)

    # 3. Compute Savings Percentage based on verified retained allocation
    ...
    print("-" * 80)
    print(f"{'Dense Grid Baseline':<32} | {dense_total_cells:<14,d} | {dense_ram_mb:<11.2f} MB | {bytes_per_dense_cell} B")
    print(f"{'Adaptive 2.5D Quadtree (Ours)':<32} | {active_cell_count:<14,d} | {quadtree_ram_mb:<11.4f} MB | {effective_bytes_per_cell:.0f} B (retained)")
    print("=" * 80)
